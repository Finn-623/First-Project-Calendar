...
- 当前版本：v0.1.2（已正式上线，2026-07-28 11:53 Australia/Sydney）
- 当前状态：本地代码、版本号、自动化测试与发布文档已完成正式上线收尾；生产环境验证已完成。
- 判断结论：v0.1.2 汇总 `aff3d4b..aa088f8` 的 134 个已完成提交，并已于 2026-07-28 11:53（Australia/Sydney）正式上线。
...
4. Roadmap 中模板、周计划、自动生成、减脂模式、离线和 AI 能力继续归入后续版本，不纳入 v0.1.2。
...
已实现但需要修复或补充测试：
- 账号切换：Store 层有 `switchAccount`，但页面侧缺少可见入口与回归测试。
- 首页饮食记录删除链路：历史详情支持删食物并自动清理空餐，但首页当日记录未接入同等删食物入口。
- 删除全部食物后自动删除空餐：规则已在历史编辑链路落地，但首页链路与专项测试仍需补齐。
- 刷新、重新登录后的状态一致性：已有初始化逻辑与部分回归测试，仍缺少端到端验证。
- 手机端主要页面：已有适配样式与若干修复记录，仍缺少系统化验收清单与可复验结果。
- Production 最新部署版本：仓库与文档显示当前为 v0.1.2，仍需发布时做线上版本号与构建一致性复核。

尚未实现，需要纳入 v0.1.3：
- 账号切换的显式 UI 入口与完整交互闭环。
- 首页当日饮食记录的食物删除与空餐自动清理闭环（与历史详情规则一致）。
- 核心链路的端到端回归用例与发布前检查脚本化清单。
...
P0：
- 补齐账号切换入口、跳转提示和回归测试。
- 补齐首页当日食物删除能力，并在删空后自动移除空餐。
- 建立“登录/切换/删除/结束本日/回到首页”关键路径回归用例。

P1：
- 建立刷新与重新登录后的状态一致性验收脚本和结果记录模板。
- 补齐手机端主要页面验收清单（登录、首页、历史、食物库、设置）。

P2：
- 形成 Production 版本一致性核对步骤（版本号、标签、部署地址、页面展示）。
