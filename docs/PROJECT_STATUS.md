...
- 个人食品体验：代码和 Migration 031/032 已完成；本轮将我的食品顶部筛选改为紧凑横向 chips，使用完整正式一级分类；个人食品分量支持数量、个/份/瓶/片/杯/勺/袋/盒/碗/条和对应克数，保存生成如 `1瓶`、`1份` 的 portion。新建默认一行，AddFoodSheet 个人食品优先；食品列表按用户/参数内存缓存、并发请求合并，公共首批异步加载；P0 编号8 已完成 AddFoodSheet 搜索、紧凑结果行、数量/克重同步和固定底部添加操作区；全量281项和Build通过，等待真实账号最终 UI 验收。
