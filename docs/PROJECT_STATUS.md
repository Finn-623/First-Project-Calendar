...
- P0-1食品记录持久化：`代码修复完成，等待用户人工验收`。首页添加食品现在以`timeline_items`/`food_entries`成功写入及数据库返回真实ID为成功标准；当日重新加载会恢复食品快照，覆盖刷新、离页返回和重新登录。远程Feedback仍为pending，尚未push或部署。
...
- P0-1“记录后返回首页记录消失”代码修复完成，等待用户人工验收。
