...
- Migration 034 `feedback_number_priority` 已部署，远程与本地一致；Migration 028 仍未部署，Migration 029-033保持一致。
- 建议编号由数据库按年份计数器并发安全生成，格式为 `FB-YYYY-NNNN`；历史记录按 `created_at` 年份、再按 `created_at,id` 稳定排序回填，编号唯一、非空、不可修改，已发出的编号不会因测试数据清理而回退或复用。
- 优先级正式定义：P0＝最快速完成；P1＝下个版本上线前完成；P2＝本大版本完成；P3＝后续版本完成。默认优先级为 P2。
- 只有管理员可通过 `set_version_feedback_priority` 调整优先级；数据库根据 `auth.uid()` 写入 `priority_assigned_by`，并生成 `priority_assigned_at`。管理员可调整 completed 建议的优先级，但 completed 标题和描述仍保持只读。
- 远程验收：实际反馈总数21；空编号、重复编号、空/非法优先级、格式错误均为0。普通用户正常登录后创建测试建议自动获得当前年度编号和 P2，直接更新 priority、调用管理员 RPC、修改编号均被拒绝；管理员正常登录后 P2→P1→P3、审计字段和 P0>P1>P2>P3 排序验证通过；4条测试建议已精确清理，原有21条逐字段未改变，远程 Feedback 状态仍保持 `pending`。
- 当前状态：代码、迁移、远程验收和自动化测试已完成，等待用户进行编号7最终页面验收；未执行 push。
