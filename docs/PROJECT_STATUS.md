...
- 进度：8/9（修改意见子页面返回入口已统一指向版本信息；全部 9 项完成后统一 push）。
...
- 新增上线前需求：退出账号入口移动到设置页最底部已完成。
  - 原“账号”分组中的退出入口已移除；账户、个人信息、版本信息、记录分组和底部版本链接之后只保留一个退出入口。
  - 退出区与前一内容保持独立间距，页面使用 `pb-32` 并叠加底部安全区留白，避免被底部导航遮挡。
  - 现有确认弹窗、取消、退出防重复、失败提示、私有状态清理、公共食品保留和 replace 返回普通登录页逻辑均未改变。
  - 专项测试：5 个套件、38 个用例通过；全量测试：30 个套件、200 个用例通过；Production build 通过。
