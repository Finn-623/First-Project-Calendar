...
- P0编号20代码完成：公共/个人标签、只读/本人可编辑边界、公共食品复制入口及添加Sheet来源区分已接通。Migration 029通过`auth.uid()`原子复制营养、alias和portion，并以来源唯一索引保证重试幂等。
- Migration 029尚未部署，远程普通用户复制与跨账号真实验收待后续明确部署授权；P0编号20继续标记为等待用户最终验收。
