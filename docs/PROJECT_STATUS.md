...
- 正式P1完整列表：7、9、16、17、21、31。
- 编号7：代码完成。已实现pending/completed独立页面、pending分页、管理员priority修改和编号/优先级展示；待部署后页面验收。
- 编号9：代码完成。已修复移动端标题、描述、priority和版本选择控件自动放大及横向溢出；待真实iPhone验收。
- 编号16：已完成。建议历史支持查看更多/分页，pending与completed结构已分离。
- 编号17：代码完成。新会话进入首页、应用内导航稳定、同标签刷新保留当前路由、退出再登录回首页；待部署后启动行为验收。
- 编号21：代码完成。食物库标题已简化；待部署后在320/375/390px及桌面端验收搜索、筛选、分页。
- 编号31：代码完成。未来日期固定三餐显示“计划餐次”而非未完成；待部署后验收未来事件、刷新和切换日期行为。
- 真正未开发的P1：无。以上未完成项均为部署后的页面、真机或真实登录态验收，不得提前标记为completed。
- 额外待验收功能：结束事件。代码已完成，右侧仅保留“结束事件”主按钮，编辑/删除降级为内容下方小字；Realtime、缓存、历史逻辑测试通过，仍待真实登录态写入、刷新和历史查看验收，不计入原P1编号。

### 统一部署后验收顺序

1. 编号7：pending总数、pending分页、completed独立页面、管理员priority修改。
2. 编号9：iPhone标题/描述不放大、键盘收起不保留zoom、无横向overflow。
3. 编号17：新标签/新会话首页、应用内导航不回首页、刷新保留路由、退出再登录首页。
4. 编号21：食物库标题、320/375/390px、搜索/筛选/分页。
5. 编号31：未来普通事件、固定三餐显示“计划餐次”、不显示未完成、刷新/切换日期。
6. 结束事件：右侧唯一结束主按钮、编辑/删除下方小字、结束后保留、状态持久化、刷新保持、历史可查看。

- 本次盘点不新增功能、不修改业务逻辑、不新增或部署Migration、不部署Production、不执行push。
- Migration 028仍未部署；远端列表显示本地029-035均与远端一致。

## 记录首页普通事件卡片操作层级调整

- 已完成：普通事件右侧只保留醒目的“结束事件”主按钮；编辑和删除移动到卡片内容下方，以无背景小字操作显示，分别使用中性色和警示色，并保留至少40px点击区域。
- 已结束事件：右侧不再显示“结束事件”，保留“已结束”Badge以及下方编辑、删除入口。
- 业务逻辑：仅调整UI层级，未改变结束、编辑、删除、Realtime、IndexedDB或历史记录逻辑；早餐、午餐、晚餐、加餐、训练和其他非普通事件保持原有操作布局。
- 验证：结束事件专项1套件5项、前端全量43套件309项、Production Build、编辑器错误检查和`git diff --check`通过；320/375/390px真实登录态UI验收待部署后完成。
- Migration/发布：未新增或部署Migration，Migration 028仍未部署；未执行远程写入验收；未执行push。
- 功能Commit：`622b1627d0a9cd9ee4c155e8f4541c2b07bfb1de`；文档Commit将在本条记录提交时生成。

## 记录首页普通事件“结束事件”

- 已完成：普通`other`事件在Today时间轴卡片操作区显示“结束事件”；点击后经过轻量确认，复用现有`status=completed`和`ended_at`字段写入真实结束时间，事件保留在时间轴和历史记录中。
- 状态展示：未结束事件显示按钮；结束事件显示“已结束”Badge并隐藏按钮；早餐、午餐、晚餐、加餐、训练和其他特殊记录不使用该按钮。删除仍代表永久删除，编辑仍保持原逻辑。
- Future规则：未来事件不会按事件时间自动结束；只有用户主动点击后才写入`ended_at`，未来日期手动结束与今天一致。
- 同步与缓存：复用现有用户归属更新、Supabase Realtime更新订阅和IndexedDB完整快照，状态刷新后可恢复；History/HistoryDetail已有`status/ended_at`标准化读取，无数据库结构变更。
- 验证：结束事件专项4套件15项、既有TodayPage专项4套件25项、前端全量43套件307项和Production Build通过。真实登录态远程验收待用户完成，当前共享浏览器停留在登录页。
- Migration/发布：未新增或部署Migration，Migration 028仍未部署；未修改公共食品及审核状态；未执行push。
- 功能Commit：`a898b94e14f8535587a03536d2ceda1f842cd552`；文档Commit将在本条记录提交时生成。

## P1 编号31：未来日期空固定餐次显示为未完成

- 已完成：确认普通未来事件只写入`timeline_items.item_type=other`，不会创建早餐、午餐或晚餐数据库行；问题来自Store加载日期时为所有日期补齐空固定餐次的前端占位。
- 修复：未来日期空固定餐次显示“计划餐次，尚未开始”，时间轴标题显示“计划时间轴”；未来日期仍可从占位餐次主动添加食品，已有食品和普通事件正常显示。今天与历史日期保持原有行为。
- 功能Commit：`cdefee3d512ccb73726c0631f7b0d155dc243a23`。
- 验证：编号31专项涉及未来日期提示、事件、食品持久化/删除、餐次时间编辑、Timeline Service、缓存和Realtime，共7套件36项通过；编辑器错误检查和`git diff --check`通过。完整前端测试、Production Build和真实登录态人工验收待执行。
- 限制：未修改数据库、Migration、公共食品或审核状态；Migration 028仍未部署；未执行push。功能等待用户在真实登录态验收未来日期、刷新、切换日期及主动添加未来餐次食品。

## P1 编号21：简化食物库页面标题和顶部信息层级

- 已完成：主页面从`LIBRARY`眉标、“食物数据库”标题和“公共食品 + 个人食品统一管理”副标题，精简为唯一主标题“食物库”；公共/我的食品继续由Tab区分。
- 保留功能：公共食品搜索、分类筛选、intake type筛选、结果数、分页、详情和复制；我的食品新增入口、使用中/已停用、搜索、分类筛选及停用/启用/永久删除；管理员审核页仍独立显示“公共食品审核”。
- 移动端：顶部高度收紧，搜索和筛选更靠前；页面根节点增加横向溢出保护，320/375/390px使用同一稳定布局规则。
- 验证：食物库专项5套件46项、前端全量42套件301项、Production Build、编辑器错误检查和`git diff --check`通过。真实登录态四视口人工验收待用户完成。
- 功能Commit：`3901f34c3a8ed4e92a38673966091074fdbab98f`。编号7、9、17继续待部署后验收；编号16保持已完成；远程Feedback仍为pending 19、completed 4。

## P1 编号17：重新进入程序默认首页

- 已完成：认证会话首次在当前浏览器标签页恢复时默认使用`replace('/')`进入首页；同一标签页刷新保留当前路由，应用内导航不触发重复跳转；退出后再次登录会重新进入首页。
- 实现：`frontend/src/App.js`使用`sessionStorage`保存一次性入口标记，并在认证路由初始化完成前避免受保护页面闪现；登录成功原有首页跳转保持不变。
- 日期规则：未修改Store的首页日期初始化、今日完成后选择下一日、删除今日历史后恢复真实今日逻辑。
- 验证：App专项7项、前端全量42套件300项和Production Build通过；编辑器错误检查与`git diff --check`通过。
- 功能Commit：`b6bf251`。真实浏览器的新标签页、重新登录、刷新和应用内导航验收待用户完成。
...
- 远程当前分布：P0=5、P1=6、P2=7、P3=5；pending 19、completed 4，总数23。
- 六条目标记录的 `submitted_priority`、编号、标题、描述、状态、target_version和completed字段均未变化；六条记录的 `priority_assigned_at`和`priority_assigned_by`已由当前管理员RPC写入。
- 本轮未修改代码、未新增或部署Migration、未修改034/035、未修改公共食品及审核状态；028仍未部署，未执行push。编号7页面仍等待用户最终确认。

### 2026-08-06 编号30、31最终校准

- 编号30“事件情况确定”已通过管理员 RPC 从P1更新为P2；编号31“未来已经添加事件后出现未完成固定三餐”已确认原P1正确，保持P1且未调用更新。
- 最终分级：P0为5、6、8、19、20；P1为7、9、16、17、18、21、31；P2为1、2、3、4、10、11、12、13、14、15、30；P3暂无。
- 远程当前分布：P0=5、P1=7、P2=11、P3=0；pending 19、completed 4，总数23。
- 030、031的 `submitted_priority` 均为P1且未修改；编号、标题、描述、状态、target_version和completed字段均未变化。030仅更新当前priority及其审计时间，审计归属仍为当前管理员；031的priority、审计字段和更新时间均未变化。
- 本轮未修改代码、未新增或部署Migration、未修改公共食品及审核状态；034/035保持远程一致，028仍未部署，未执行push。编号7页面仍等待用户最终确认。

### 2026-08-06 现有反馈 priority 校准

- 已按用户确认的正式定义通过管理员 RPC 校准当前执行 priority：P0（最快速完成）为编号5、6、8、19、20；P1（下个版本上线前完成）为编号7、9、16、17、18、21；P2（本大版本完成）为编号10-15。编号1-4在审核文档中标记为Closed且远程已completed，保持原P2；没有文档依据的P3记录。
- 远程当前分布：P0=5、P1=8、P2=10、P3=0；pending 19、completed 4，总数23。该记录形成时P1计数包含新增030、031的原始P1值；最终结果由上方“编号30、31最终校准”记录。
- 实际通过管理员 `set_version_feedback_priority` RPC 更新11条，编号10-15原本已正确未更新。`submitted_priority`、编号、标题、描述、状态、版本和完成字段全部保持不变；变更记录审计字段已由当前管理员更新。
- 新增 `FB-v0.1.3-030`和`FB-v0.1.3-031`当时无法明确对应现有审核编号，后续最终确认分别为P2和P1，详见上方最新记录。
- 本轮未修改代码、未新增或部署Migration、未修改公共食品及审核状态；034/035保持远程一致，028仍未部署，未执行push。编号7页面仍等待用户最终确认。

### 2026-08-06 建议历史结构修复

- 主页面默认进入“未完成建议”，服务层按 `status='pending'` 使用数据库精确总数和每页10条范围查询；标题与分页均使用数据库总数，待处理记录不会混入已完成记录。远程当前 pending 总数为19，第一页10条，最后一页9条。
- 已完成建议从 `/settings/version/feedback/completed` 独立进入，服务层按 `status='completed'` 独立查询并显示数据库总数，不参与待处理分页。远程当前 completed 总数为4。
- 本轮专项37项、前端全量42套件298项、Production Build、编辑器检查和真实远程验收均通过；用户仍需重新完成编号7页面验收。未新增Migration、未部署、未push。

- 2026-08-06 用户复验发现两个真实问题：历史页原先通过 `limit(10)` 和“查看更多”加载，未提供总数和分页；管理员判断遗漏 `profile.account_type=admin`，导致管理员按普通用户加载并看不到现存反馈 priority 控件。现已修复为一次查询完整排序结果、按每页10条分页，并补充总数、页码、上一页/下一页；管理员可修改所有用户的 pending/completed 当前 priority，失败显示错误，成功后立即刷新并重新排序。
- 远程复验时发现旧21条基线之外新增2条真实 pending 建议（编号 `FB-v0.1.3-030`、`FB-v0.1.3-031`），因此当前真实总数为23，页面实际应显示第1页10条、第2页10条、第3页3条。两条新建议未删除、未过滤；远程原有内容、状态和最终 priority 保持不变。
- 034 原实现与最终需求存在偏差：使用 `FB-YYYY-NNNN` 且默认 P2。已保持034历史文件不变，通过 Migration 035 完成兼容纠正；035已远程部署，028仍未部署。
- 用户提交时必须自行选择 P0/P1/P2/P3：P0＝最快速完成；P1＝下个版本上线前完成；P2＝本大版本完成；P3＝后续版本完成。用户选择保存于不可变 `submitted_priority`，当前执行优先级保存于 `priority`；初次提交二者相同，管理员可以调整 `priority`，但不能改变 `submitted_priority`。
- 建议编号由数据库按 `target_version` 的版本计数器和事务锁生成，格式为 `FB-v<版本号>-<三位版本内编号>`，例如 `FB-v0.2.1-001`。当前提交页面固定使用应用当前版本 `0.1.3`，不让用户任意填写版本；编号不可修改、删除后不复用。
- 历史21条反馈均有真实版本归属：completed使用 `completed_version=v0.1.3`，pending按当前反馈所属正式版本 `0.1.3` 回填；同版本按 `created_at,id` 升序生成版本内序号；历史 `submitted_priority` 回填为原有 priority，并记录无法恢复旧实现未保存的原始选择。
- 页面提交表单显示当前所属版本和四个完整说明的必选单选项；历史页同时显示用户提交优先级、当前执行优先级、建议编号和所属版本。管理员下拉框继续调整当前 priority，并显示审计信息。
- 远程验收：普通/管理员正常 username-login、P0-P3并发提交、普通用户越权拒绝、管理员调整审计、completed兼容、同版本/跨版本编号、删除不复用、P0-P3再按created_at排序均通过；总反馈21，空编号、重复编号、缺版本、非法格式、非法 submitted_priority/current priority均为0；临时数据残留0，原有反馈状态保持 `pending`。
- 当前状态：代码、035部署、远程验收和自动化测试完成，等待用户重新进行编号7最终页面验收；未执行 push。
...
- 个人食品体验：代码和 Migration 031/032/033 已完成；个人食品支持完整分类、常用分量、按真实用户缓存的 active/inactive 状态；新增明确的停用、重新启用、永久删除操作和“使用中/已停用”筛选。Migration 033 的永久删除 RPC 仅允许删除当前用户、未被任何 food_entries 引用的 private 食品，并在单事务中清理 alias、portion和收藏；AddFoodSheet 只读取 active，历史快照不受影响。P0 编号8 的个人食品查询、缓存和操作链已完成，前端全量288项和Build通过，等待真实页面最终 UI 验收。
