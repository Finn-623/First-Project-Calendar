...
for (const id of ids) {
  ...
  if (/\bunpeeled\b|\bwith skin\b/.test(english)) {
    assert.match(translation.name_zh, /带皮/, `${id} skin retained`);
  }
  if (/\bpeeled\b|\bwithout skin\b/.test(english)) {
    assert.match(translation.name_zh, /去皮/, `${id} peeled retained`);
  }
  if (/\bregular fat\b/.test(english)) {
    assert.match(translation.name_zh, /全脂/, `${id} regular fat retained`);
  }
  if (/\breduced fat\b/.test(english)) {
    assert.match(translation.name_zh, /低脂/, `${id} reduced fat retained`);
  }
  if (/\bskim\b/.test(english)) {
    assert.match(translation.name_zh, /脱脂/, `${id} skim retained`);
  }
  if (/\bcanned in water\b/.test(english)) {
    assert.match(translation.name_zh, /水浸/, `${id} water-packed retained`);
  }
  if (/\bdrained\b/.test(english)) {
    assert.match(translation.name_zh, /沥干/, `${id} drained retained`);
  }
}
...
for (const [name, group] of duplicateNames) {
  if (group.length > 1) {
    assert.ok(
      group.every((item) => item.translation_status === 'needs_review'),
      `ready duplicate Chinese name: ${name}`
    );
...
const englishNames = Object.values(translations).filter((item) =>
  /[A-Za-z]{2,}/.test(item.name_zh)
);
