-- Atomically create or update one user's private food and its portions.
...
BEGIN
  ...
  FOR portion_row IN SELECT value FROM jsonb_array_elements(COALESCE(p_portions, '[]'::JSONB)) LOOP
    ...
    END IF;
    IF portion_grams IS NULL OR portion_grams <= 0 THEN
      RAISE EXCEPTION 'portion grams must be greater than zero';
    END IF;
    IF EXISTS (
      SELECT 1
  ...
  SELECT food_row.id, value->>'name', (value->>'grams')::NUMERIC, (value->>'isDefault')::BOOLEAN
